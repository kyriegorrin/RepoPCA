...
OUTPUT_NAME_2="executionsAux.dat"
...
if diff $ORIGINAL.out $OPTIMIZED.out > /dev/null; then 
	...
	echo -e "OUTPUTS DIFERENTS. DANI SIUSPLAU.\n"
	exit 0
fi
...
while [ $CONTADOR -le $NMAX ]; do
	...
	#Passem valors al fitxer output 1
	echo "$CONTADOR $AVG_ELAPSED" >> $OUTPUT_NAME

	#Utilitzem AVG_ELAPSED temporalment per a generar segon output
	AVG_ELAPSED=$(bc <<< "scale=4;$AVG_ELAPSED/$CONTADOR")

	#Generem arxiu output 2
	echo "$CONTADOR $AVG_ELAPSED" >> $OUTPUT_NAME_2
...
gnuplot -p -e 'plot "executionsAux.dat" with linespoints; q'
